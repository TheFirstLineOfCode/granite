...
* **Granite Lite**<br>
使用Granite Lite部署模式，系统被部署在一个单节点（物理机、虚拟机、容器）上，仅依赖JVM。<br><br>
...
Granite Cluster部署版本持久层使用MongoDB数据库，而Granite Lite部署版本使用一个嵌入式的数据库HSQL Server。

<br><br>
### 3 Granite Lite部署
#### 3.1 前置准备
* **安装JDK或JRE**<br>
Granite Lite依赖JDK 11+或者JRE 11+。你需要将JDK或者JRE先安装到系统中。
...
<br><br>
##### 3.5.4 配置服务器组件配置参数
可以在components.ini配置文件中，修改服务器组件的配置参数。<br><br>
例如，我们需要修改XMPP Server启动端口号为5333（默认端口号为5222），可以在components.ini文件中socket.message.receiver章节，设置参数port：
```
...
![](https://dongger-s-img-repo.oss-cn-shenzhen.aliyuncs.com/images/granite_cluster_architecture.png)

<br><br>
#### 4.2 部署Granite Cluster
...
![](https://dongger-s-img-repo.oss-cn-shenzhen.aliyuncs.com/images/granite_cluster_deploying.png)

<br><br>
#### 4.2.2 安装Granite Cluster MgtNode
##### 4.2.2.1 前置准备
* **安装JDK或JRE**<br>
Granite Cluster MgtNode依赖JDK 11+或者JRE 11+。你需要将JDK或者JRE先安装到系统中。

<br><br>
##### 4.2.2.2 下载Granite Cluster MgtNode
...
<br><br>
* **设置数据库参数**<br>
Granite Cluster使用MongoDB数据库，假设我们已经安装了MongoDB，并创建了用户，需要在deploy-plan.ini的db章节配置数据库相关信息：
>>```
>>[db]
...
$
```
可以在MgtNode Console中执行exit命令，终止MgtNode的运行。
```
$ exit
```

<br><br>
#### 4.2.3 安装Granite Cluster AppNode
##### 4.2.3.1 前置准备
* **安装JDK或JRE**<br>
Granite Cluster MgtNode依赖JDK 11+或者JRE 11+。你需要将JDK或者JRE先安装到系统中。
...
java -jar granite-cluster-nodes-appnode-1.0.5-RELEASE.jar --rt-console
```
